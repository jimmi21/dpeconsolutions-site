...
// ---------- Calculator (unchanged logic with careful wording) ----------
const TAX_TABLES = {
...
function calculate(){
  ...
  try{ localStorage.setItem('dpas_calc_v32', JSON.stringify({year,advRate,duty,salary,business,rental,children,prepaid,credits})); }catch{}
}
...
document.getElementById('printBtn')?.addEventListener('click',()=>{
  ...
  const css = `
    ...
    .brand img{height:88px;border-radius:12px}
    .muted{color:#555}
...
// Compute on load
(function(){ try{ const saved=localStorage.getItem('dpas_calc_v32'); if(saved){ const s=JSON.parse(saved);
  ['taxYear','advanceRate','businessDuty','incSalary','incBusiness','incRental','children','prepaid','credits']
  .forEach(id=>{ const el=document.getElementById(id); if(el && s[id.split(/(?=[A-Z])/).join('').toLowerCase()]!==undefined) el.value = s[id]; }); }
} catch{}; })();
calculate();
