...
// Theme — default dark; includes 'mono' (black-white-gold), grey, greige, light
...
function calcProgressiveDetailed(amount, brackets){
  ...
  for(const b of brackets){
    ...
    if(part>0){
      ...
      rows.push({range:b.upTo===Infinity?`${fmt(prev+1)}+`:`${fmt(prev+1)}–${fmt(b.upTo)}`, amount:part, rate:b.rate, tax});
...
function toCSV(rows){ const head=['Κλιμάκιο','Ποσό','Συντελεστής','Φόρος']; const body=rows.map(r=>[r.range,r.amount,(r.rate*100).toFixed(0)+'%',Math.round(r.tax)]); return [head,...body].map(l=>l.join(';')).join('\\n'); }
...
function calculate(){
  ...
  try{ localStorage.setItem('dpas_calc_v3', JSON.stringify({year,advRate,duty,salary,business,rental,children,prepaid,credits})); }catch{}
}
...
document.getElementById('printBtn')?.addEventListener('click',()=>{
  ...
  const css = `
    ...
    .brand img{height:64px;border-radius:12px}
    .muted{color:#555}
  ...
  <body>
    ...
    <div class="grid">
      ...
      <div>
        ...
        <table><thead><tr><th>Κλιμάκιο</th><th>Ποσό</th><th>Συντ.</th><th>Φόρος</th></tr></thead><tbody>${tbodyHTML('#tblSalary tbody')}</tbody></table>
        <h3>Επιχειρ. δραστηριότητα</h3>
        <table><thead><tr><th>Κλιμάκιο</th><th>Ποσό</th><th>Συντ.</th><th>Φόρος</th></tr></thead><tbody>${tbodyHTML('#tblBusiness tbody')}</tbody></table>
        <h3>Ακίνητα</h3>
        <table><thead><tr><th>Κλιμάκιο</th><th>Ποσό</th><th>Συντ.</th><th>Φόρος</th></tr></thead><tbody>${tbodyHTML('#tblRental tbody')}</tbody></table>
      </div>
    </div>
